...
##assigned features to columun name of the HAR Dataset
colnames(hardata) <- features
##select mean and std columns from HAR dataset
features <- data.frame(features)
...
harmean <- hardata[,meannames]
harstd <- hardata[,sdtnames]
har<- cbind(harmean, harstd, hardata[562], hardata[563])##bind together meand & std, along with activity and subject data
##add column names for activity and subject
names(har)[80] <- "activity" ## add column name 
names(har)[81] <- "subject" ## add column name 
